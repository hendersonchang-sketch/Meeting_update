...
export async function analyzeMeetingVideo(
  ...
  try {
    ...
    const analysisResultData = await client.models.generateContent({
      model: 'gemini-3-flash-preview',
      config: {
        systemInstruction: `你是南山人壽技術小組的專業會議秘書，負責根據逐字稿整理高度精確且「條列化」的會議記錄。
        嚴格遵守以下準則：
        1. 內容詳盡度：總字數需求需大於 1000 字（包含摘要與細節），但「重點紀錄」部分必須以「條列式短句」呈現。
        2. 隱私規範：嚴禁在報告中提及任何具體的人名。
        3. 重點紀錄分類：必須將所有討論重點精確分配到以下 7 個固定分類。
        4. 內容精簡化（關鍵）：
           - 嚴禁寫成段落描述（作文形式）。
           - 每一項內容 (content) 必須是「獨立的短句」，類似清單項目。
           - 移除轉場詞（如：關於...、在...方面、此外...、為了...）。直接陳述事實項目。
        5. 語言要求：使用繁體中文。
        6. 術語修正：若逐字稿出現「四方機房」，必須在報告中「直接且安靜地」修正為「是方機房」。
        7. 絕對禁止 Meta-Commentary：輸出內容中不准出現任何關於指令執行、規則確認的文字。
        8. 格式規範：JSON 內容中嚴禁使用項目符號或數字編號。
        9. 結構完整性：確保 JSON 輸出完整，優先保證 minutes 區塊的完整性。`,
        maxOutputTokens: 32768,
        temperature: 0.1,
